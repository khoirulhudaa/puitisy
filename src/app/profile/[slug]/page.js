...
import { getBookDetail } from '@/redux/auth/DataSlice'
...
const Page = () => {
  ...
  useEffect(() => {
    const getAccount = async () => {
      const response = await url_endpoint.getAccountById(auth?.user_id)
      const responseBook = await url_endpoint.getBookByAuthorId(auth?.user_id)
  ...
  return (
    <section className='w-screen min-h-[90vh] overflow-hidden'>
        <div className='relative overflow-hidden flex items-center justify-center w-full h-[45vh] bg-slate-900'>
            ...
            <Link href={`/profile/edit/${slug}/${auth?.user_id}`}>
        ...
        <div className='relative w-full px-10 py-5'>
            ...
            <div className={`w-full mt-8 flex flex-wrap ${Array.isArray(books) && books.length > 3 ? 'justify-between' : 'justify-start'}`}>
              <Link href={`/upload-book/${slug}/book`} className={`w-[23%] mb-5 flex ${books.length > 3 ? 'mr-0' : 'mr-3'} items-center justify-center flex-col h-[350px] rounded-[12px] bg-white border border-slate-300 shadow-sm cursor-pointer active:scale-[0.98] duration-200 overflow-hidden`}>
              ...
              </Link>
              {
                Array.isArray(books) && books.length > 0 ? (
                  books.map((_, index) => (
                  <div className={`w-[23%] mb-5 h-[360px] ${books.length > 3 ? 'mx-0' : 'mx-3'}`}>
                    <div key={index} className="w-full mb-5 h-full rounded-[12px] bg-white border border-slate-300 shadow-sm cursor-pointer duration-200 overflow-hidden">
                      ...
                      <div className="relative w-full h-[60%] overflow-hidden object-cover">
                        ...
                        <Link href={`upload-book/${slug}/book/`}>
                          <div className='absolute flex bg-white rounded-full hover:brightness-90 active:scale-[0.98] duration-100 px-2 py-1 shadow-md border border-white items-center bottom-4 right-4'>
                            ...
                            <p className='text-[14px]'>Create poem</p>
                          </div>
                        </Link>
